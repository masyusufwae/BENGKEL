        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="font-bold text-2xl text-black">
                    Riwayat Servis (Arsip)
                </h2>
                <p class="text-sm text-gray-500 mt-1">Pekerjaan Work Order yang sudah selesai</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('mekanik.work-order.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 text-sm">
                    ← Semua Work Order
                </a>
            </div>
        </div>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-xl border border-gray-100 p-6">
                <form method="GET" action="{{ route('mekanik.riwayat') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    {{-- Filter Tanggal --}}
                    <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('mekanik.riwayat') }}" class="flex-1 bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 font-medium text-center">
                            Reset
                        </a>
                    </div>
                </form>
...
<td colspan="7" class="text-center py-12 text-gray-500">
                                        <div class="space-y-2">
                                            ...
                                            <p class="text-sm">Selesaikan beberapa Work Order untuk melihat arsip di sini.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
