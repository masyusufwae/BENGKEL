...
<div class="mb-8 flex justify-between items-center">
    ...
    <a href="{{ route('customer.orders.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
...
<div class="space-y-4">
    @forelse($workOrders as $wo)
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ $wo->nomor_wo }}</h3>
                    <p class="text-sm text-gray-600">
                        {{ $wo->tanggal_masuk ? \Carbon\Carbon::parse($wo->tanggal_masuk)->format('d M Y') : '-' }}
                    </p>
                </div>

                @php $invoice = $wo->invoice->first(); @endphp

                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                    {{ $invoice && $invoice->status_bayar == 'lunas' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                    {{ $invoice ? ucfirst($invoice->status_bayar) : 'Belum ada invoice' }}
                </span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 border-t border-gray-200 pt-4">
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 mt-1">
                        {{ $wo->kendaraan->merek ?? 'N/A' }} {{ $wo->kendaraan->model ?? '' }}
                    </p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 mt-1">
                        Rp {{ number_format($invoice->total_bayar ?? 0, 0, ',', '.') }}
                    </p>
                </div>
                <div class="text-right">
                    <a href="{{ route('customer.orders.show', $wo->nomor_wo) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                        Lihat Detail →
                    </a>
                </div>
            </div>
    ...
    @empty
        <div class="bg-white rounded-lg shadow p-12 text-center">
            ...
            <a href="{{ route('customer.orders.create') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                </svg>
